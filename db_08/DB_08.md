...
```python
info = Director.objects.get(name="봉준호")
print(f"id : {info.id}")
...
```python
info = Genre.objects.get(title="드라마")
print(f"id : {info.id}")
...
import datetime
movie = Movie(
    director = director_.id,
    genre = genre_.id,
    title = '기생충',
    opening_date = datetime.date(2019, 5, 30),
    running_time = 132, 
    screening = False
)
movie.save()
...
for mov in movies:
    movie = Movie(
        director = Director.objects.get(name=mov[0]),
        genre = Genre.objects.get(title=mov[1]),
        title = mov[2],
        opening_date = mov[3],
        running_time = mov[4], 
        screening = mov[5]
...
movie = Movie.objects.all()
for i in movie:
    print(Director.objects.get(id=i.director_id).name, i.genre, i.title, i.opening_date, i.running_time, i.screening)
```
...
movie = Movie.objects.all()
for i in movie:
    print(Director.objects.get(id=i.director_id).name, Genre.objects.get(id=i.genre_id).title, i.title, i.opening_date, i.running_time, i.screening)
```
...
```python
movie = Movie.objects.filter(director_id = Director.objects.get(name='봉준호')).order_by('opening_date')
for i in movie:
    print(Director.objects.get(id=i.director_id).name, Genre.objects.get(id=i.genre_id).title, i.title, i.opening_date, i.running_time, i.screening)
```
...
```python
movie = Movie.objects.filter(director_id = Director.objects.get(name='봉준호')).order_by('opening_date')[1:2]
i = movie[0]
print(Director.objects.get(id=i.director_id).name, Genre.objects.get(id=i.genre_id).title, i.title, i.opening_date, i.running_time, i.screening)
```
...
```python
movie = Movie.objects.filter(opening_date__gt=datetime.date(2022,1,1)).order_by('opening_date')
for i in movie:
    print(Director.objects.get(id=i.director_id).name, Genre.objects.get(id=i.genre_id).title, i.title, i.opening_date, i.running_time, i.screening)
```
