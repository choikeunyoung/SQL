...
| 단, 컬렴명을 `송장수`로 출력하세요.
```sql
SELECT COUNT(*) AS '송장수' FROM invoices WHERE BillingPostalCode != 'NULL' ORDER BY InvoiceDate DESC LIMIT 5;
송장수
---
384
```
...
```sql
SELECT COUNT(*) FROM invoices WHERE strftime('%Y',InvoiceDate) = '2013' ORDER BY InvoiceDate;
COUNT(*)
--------
80
```
...
```sql
SELECT COUNT(*) AS "고객 수",Country, State FROM customers WHERE State != ' ' GROUP BY Country, State ORDER BY "고객 수" DESC, Country DESC LIMIT 5;
고객 수  Country  State
----  -------  -----
3     USA      CA
3     Brazil   SP
...
```sql 
SELECT CustomerId, CASE WHEN Fax IS NULL THEN 'X' WHEN Fax IS NOT NULL THEN "O" END 'Fax 유/무' FROM customers ORDER BY CustomerId ASC LIMIT 5;
CustomerId  Fax 유/무
----------  -------
1           O
2           X
